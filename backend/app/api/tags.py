...
async def list_tags_endpoint(
  ...
  tags, total = await list_tags(
    session, page=page, page_size=page_size, search=search, user_id=scoped_user_id(user)
  )
...
async def get_tag_endpoint(
  tag_id: int, user: CurrentUser, session: AsyncSession = Depends(get_db)
):
...
async def update_tag_endpoint(
  ...
  if tag_update.name is not None:
    tag = await update_tag(
      session, tag_id, tag_update.name, user_id=scoped_user_id(user)
    )
  else:
    tag = await get_tag_or_404(session, tag_id, user_id=scoped_user_id(user))
  return TagSchema.model_validate(tag)
